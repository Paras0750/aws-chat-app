...
export const ChatClient = (props: Props) => {
  return (
    <div style={{
      ...
      <CssBaseline />
      <Container maxWidth="lg" style={{ height: '80%' }}>
      <div style={{textAlign: "center",fontSize: "2rem", marginBottom: "20px",fontWeight: 600}}>Chat App</div>
        <Grid container style={{ height: '100%' }}>
          <Grid item xs={2} style={{ backgroundColor: "#9EB6D9", color: 'white', padding:"20px" }}>
            <List component="nav">
              {props.members.map(item =>
                <ListItem key={item} onClick={() => { props.onPrivateMessage(item); }} button>
                  <ListItemText style={{ fontWeight: 100, color: "#000" }} primary={item} />
                </ListItem>
              )}
            </List>
          </Grid>
          <Grid style={{ position: 'relative' }} item container direction="column" xs={10} >
            <Paper style={{ flex: 1 }}>
              <Grid item container style={{ height: '100%'  }} direction="column">
                ...
                <Grid item style={{ margin: 10, padding: 10 }}>
